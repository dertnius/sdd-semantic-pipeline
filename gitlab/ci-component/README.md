...
on the GitLab instance** via `include: component:`. Currently four components:
...
1. Create a GitLab project `<group>/ci-components` and push the **contents of this
   ...
   `templates/docx-to-chunks/template.yml`, `templates/validate-skills/template.yml`, and
   `templates/sharepoint-download/template.yml` sit under the repo root, and the `scripts/`
   dir alongside them).
2. Add a project description + this README, then **Settings → CI/CD → CI/CD Catalog
   resource → ON**.
3. Set the `NEXUS_*` variables at the **group** level.
4. Push a branch / open an MR → the `component-selftest`, `docx-to-chunks-selftest`, the
   self-included `validate:skills`, and `sharepoint-download-selftest` jobs validate the
   components at `@$CI_COMMIT_SHA` (Nexus image, pandoc, a trivial install, a docx → chunks
   smoke test, a SKILL.md check against the committed
   `templates/validate-skills/examples/skills` fixture, and a YAML-merge + PowerShell-parse
   smoke of the SharePoint downloader). The Windows `prepare` stage and the live
   SharePoint/Vault path can only be exercised on the appropriate runner / tenant.
5. Tag a semver release (e.g. `1.0.0`) → the `release` job publishes it to the org Catalog.
   All four components share the repo's tag/version. **`sharepoint-download` also needs its
